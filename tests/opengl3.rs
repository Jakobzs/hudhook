...
use hudhook::hooks::opengl3::{EguiOpenGl3Hooks, ImguiOpenGl3Hooks};
use hudhook::*;
...
fn test_egui_opengl3() {
    ...
    if let Err(e) =
        Hudhook::builder().with(HookExample::new().into_hook::<EguiOpenGl3Hooks>()).build().apply()
    {
        eprintln!("Couldn't apply hooks: {e:?}");
    }
